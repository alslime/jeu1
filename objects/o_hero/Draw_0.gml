...
if surface_exists(inst_front_surface.front_surf)
{
	...
	script_execute(text_outline,x1 + 62, y1 + 78,total_gears)
	draw_text_transformed_color(x1 + 62, y1 + 78,total_gears,1,1,0,c_white,c_white,c_white,c_white,1)
	gear_icon.strg_width = string_width(total_gears)

	draw_set_font(fontsmall)
	draw_healthbar(x1 + 32, y1 + 32, x1 + 32 + hpmax, y1 + 49, ((hpmax-hplost)/hpmax)*100,c_black,c_red,c_red,0,true,false)
	draw_healthbar(x1 + 32, y1 + 56, x1 + 32 + energymax, y1 + 73, ((energymax-energylost)/energymax)*100,c_black,c_blue,c_blue,0,true,false)
	...
	draw_text_transformed_color(x1 +32 +hpmax/2 -string_width(string(hpmax-hplost) + "/" + string(hpmax))/2, y1 + 30,string(hpmax-hplost) + "/" + string(hpmax),1,1,0,c_white,c_white,c_white,c_white,1)
	...
	draw_text_transformed_color(x1 +32 +energymax/2 -string_width(string(energymax-energylost) + "/" + string(energymax))/2, y1 + 54,string(energymax-energylost) + "/" + string(energymax),1,1,0,c_white,c_white,c_white,c_white,1)
