...
if inst_hero.chosen_support == button_name
{
	...
	if inst_hero.equip_button_chosen == id
	{
		//Equipped or not
		draw_set_font(fontbig)
		script_execute(text_outline,x1+camera_get_view_width(view_camera[0])/2+13+130 + string_width(name), y1 + 77," Equipped")
		draw_text_transformed_color(x1+camera_get_view_width(view_camera[0])/2+13+130 + string_width(name), y1 + 78," Equipped",1,1,0,c_red,c_red,c_red,c_red,1)
	}
}
