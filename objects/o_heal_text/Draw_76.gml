...
if instance_exists(o_front_surface) && inst_hero.equip_menu == false
{
	...
	{	
		...
		{
			script_execute(text_outline,x - font_get_size(fontsmall)/2, y - font_get_size(fontsmall)-1,regen_value)
			draw_text_color(x - font_get_size(fontsmall)/2, y - font_get_size(fontsmall),regen_value,col,col,col,col,1)
